...
Each worker service periodically publishes its current status to a
topic named `work-queue/worker-updates`.  The frontend services
subscribe to this topic.  All of the frontends receive a copy of each
worker update.
