...
protocol to send and receive messages.  The workers may be implemented in
...
This mission demonstrates a pattern called request/response messaging, a 
distributed messaging paradigm which uses a shared work queue.  It 
...
==== ALR Note: Let's add a paragraph noting that the client will poll for new data and receive a list of all requests; this approach may or may not fit the needs of your application as compared with event-based/push mechanisms.

== Use Case

=== Deployment

==== ALR Note: This to me reads more like documentation notes for how to use in the ZIP download case, because the launcher handles deployment when done using CI/CD Build and Deploy.  Recommend asking the docs team where this goes best.
...
payload `{"text": "abc"}` succeeds with no return value.  

* Subsequent GET requests to `http://<frontend>/api/data` contain the JSON
`{"responses":[{"workerId":"<id>","text": "ABC"}]}`, which represents 
all requests that have been processed by the system.
